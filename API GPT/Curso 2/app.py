    ...
    while True:
        try:
            ...
            if caminho_imagem_enviada != None:
                reposta_vision = analisar_imagem(caminho_imagem_enviada)
                reposta_vision += "Na resposta final apresente detalhes da imagem analisada"
            ...
            while run.status !=STATUS_COMPLETED:
                ...
                if run.status == STATUS_REQUIRES_ACTION:
                    ...
                    for tool in tools_acionadas:
                        ...
                        argumentos = json.load(tool.function.arguments)
                        print(argumentos)
                    ...
                    run = cliente.beta.threads.runs.submit_tool_outputs_and_poll(
                        thread_id=thread_id,
                        run_id=run.id,
            ...
            if repeticao >= maximo_tentativas:
                ...
                print("Erro de comunicação, tentando novamente")
                sleep(1)
...
def upload_imagem():
    ...
    return 'Não foi possível receber a imagem', 400
